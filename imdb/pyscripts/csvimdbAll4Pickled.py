...
nEntries=13997
bar = Bar('Processing', max=nEntries)
...
def storeMovieData(id,count) : 
    movie = ia.get_movie(id)
    allMovies.append(movie)
    #print("here2")

    with open('imdb4Pickled.csv', 'a') as csvfile:
        ...
        if(count%10==0 and count!=0):
            ...
            currentRate=count/(timeElapsed)
            eta=((nEntries-count)/currentRate)/60
            print "Time Elapsed : %s minutes." %(timeElapsed/60)
            print "Current Speed : %s movies/second." %(currentRate)
            print "Estimated Time to Completion : %s minutes." %eta        
            #bar.next()
    return    
...
try:
    ...
    for row in reader:   # iterates the rows of the file in orders
        ...
        thread.start_new_thread(storeMovieData,(id,count,))
        count=count+1
        if(count%100==0):
            pickle.dump( allMovies, open( "allMoviesIncremental.pickle", "wb" ) )
        time.sleep(0.15)
        #print("here3")
...
pickle.dump( allMovies, open( "allMovies.pickle", "wb" ) )
bar.finish()